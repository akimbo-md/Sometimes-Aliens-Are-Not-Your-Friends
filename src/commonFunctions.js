...
function getTextures(gl, imgPath, callback) {
    let fullpath = "/materials/" + imgPath;
    var texture = gl.createTexture();
    gl.bindTexture(gl.TEXTURE_2D, texture);
    // Initial red texture to use until the image loads
    ...
    image.onerror = function () {
        console.error('Failed to load image at path: ' + fullpath);
    };
    console.log("Texture loaded", imgPath);
    image.src = fullpath;
    return texture;
}
